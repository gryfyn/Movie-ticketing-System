...
const MoviePage = () => {
  ...
  const fetchMovies = async () => {
    ...
    if (error) {
      ...
      // Fetch dominant colors for each movie image
      data.forEach((movie) => {
        const img = new Image();
  ...
  return (
    <div className="movie-page">
      ...
      <main className="main-content">
        ...
        <div className="movie-grid" style={{ marginTop: '50px' }}>
          {movies.map((movie, index) => (
            <Link to={`/movie/${movie.id}`} key={index} className="movie-link">
              <div
                ...
                key={index}
                style={{
                  background: `linear-gradient(to bottom, ${dominantColors[movie.id] || '#333'}, #000)`
                }}
              >
                <img src={movie.image} alt={movie.title} />
                <div className="description-overlay">
                  <p style={{color: '#FFE1E9'}}>{movie.about}</p>
                </div>
                <p>{movie.title}</p>
